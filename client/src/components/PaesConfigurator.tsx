...
export default function PaesConfigurator() {
  ...
  // Precio del tutor por materia (configurable) - incluye IVA 19%
  const TUTOR_PRICE_PER_SUBJECT = 59500; // 50000 * 1.19
  ...
  // Calcular precio total (aplicando IVA 19% a los precios base)
  useEffect(() => {
    if (!subjects) return;

    const selectedSubjectsData = subjects.filter(s => selectedSubjects.includes(s.id));
    // Aplicar 19% IVA a cada precio base
                  ...
                  {selectedSubjects.length > 0 && (
                    <div className="border-t-2 border-[#002147]/20 pt-4">
                      <div className="bg-gradient-to-br from-[#002147]/10 to-[#002147]/5 rounded-2xl p-5 shadow-inner">
                        ...
                        <div className="text-right text-xs text-[#002147]/50 font-medium">* Incluye IVA</div>
                      </div>
                    </div>
                  )}
